...
| # | 项 | 前置 | 量级 | 建议 |
|---|---|---|---|---|
| 1 | `ui-e2e.yml` 升格为发版门禁 | 无 | 小 | ✅ 已完成（2026-09-19） |
| 2 | 打包版「硬杀重启」任务恢复 E2E | 无 | 中 | ✅ 已完成（2026-09-19） |
| 3 | `task_block` 调用率观察 | 需真实会话数据 | 长期 | 顺带 |
| 4 | `MessageAttachmentCard` 组件级测试 | 需先引入前端测试基础设施 | 中 | 单独决策 |
| 5 | 真机触摸屏拖拽 | 需硬件 | 小 | 挂着 |
| 6 | 交付由模型显式声明（事故驱动的修订） | 无 | 中 | ✅ 已完成（2026-09-19） |
...
## 6. 交付由模型显式声明（已完成，2026-09-19）

> **已完成，framework 0.9.0（tag `v0.9.0`），Lectern vendor 已切到 `zmzai-agent-framework-0.9.0.tgz`。**
> 详细口径见验收报告新增的 §1.5 与 §4(6)。这一项**不在原始清单里**，是一次真实事故驱动的修订：
> 用户发「继续执行」，模型回「继续。跑自检验证前面改动的正确性：」（停在冒号上、零工具调用、
> 28 个输出 token），框架照发 `task.delivered`，交付卡写「完成了 0 个步骤、0 次工具调用」。

**它修的是与规格 §3.1 同形的第二条推导。** 首轮修掉了「一次运行结束 ⇒ 任务完成」，
但仍留着「本轮有输出文本 ⇒ 验收条件通过」（`projectImplicitCriterion`）。修法不是把启发式判据
调聪明，而是取消推导本身：新增 `task_deliver` 工具，`delivered` 只由它的显式声明触发
（Completion Gate 条件 6）；删除 `projectImplicitCriterion`，替代品是纯函数 `applyDelivery`。

**顺带堵掉的一个平凡漏洞**：任务一条 required 验收条件都没有时，条件 1/2/3/5 会**全部平凡通过**，
交付又退回到「文本非空即完成」。单列条件 6 之后这条口子与条件集无关地关闭了。

**交付卡那两个数字此前在任何任务上都是错的**（`task.started` 是唯一带 `acceptanceCriteria` 的事件，
那一刻必然全 pending、证据必然 0 条），现在由 `task.delivered` 的 `delivery` / `criteria` /
`evidenceCount` 三个必填字段承载，并有浏览器 E2E（`e2e/task-delivery-ui.mjs`，走 SSE 折叠路径）
钉住卡片上实际画出的值。

**遗留**：`task_deliver` 的调用率同样值得观察（与第 3 项同源）——它是新的模型侧契约，
模型不调它任务就停不下来。如果观察到「模型做完却只输出总结、不调工具」，要先看工具 description
的措辞，而不是把它再放宽回文本判定。
