    ...
    void (async () => {
      ...
      if (options.runtime) {
        ...
        if (req.method === "POST" && url.pathname === "/v1/terminal") {
          ...
          try {
            send(res, 200, await options.realRuntime.terminalCreate(typeof body.cwd === "string" ? body.cwd : "", Math.floor(Number(body.cols ?? 80)), Math.floor(Number(body.rows ?? 24)), typeof body.command === "string" ? body.command : undefined, typeof body.sessionId === "string" ? body.sessionId : undefined));
          } catch (error) {
            send(res, 500, { error: "INTERNAL", message: error instanceof Error ? error.message : String(error) });
          }
          return;
